#!/usr/bin/env python3
"""
Full Chain Test - From Overlay User Prompt to Plan Creation and Execution
...
class FullChainTester:
    """Test the full chain from user prompt to execution"""
    ...
    async def test_execution_simulation(self, plan: Dict[str, Any]) -> Dict[str, Any]:
        ...
        total_duration = sum(step.get("estimated_duration", 1.0) for step in steps)
        execution_result["execution_time"] = total_duration
        ...
        if execution_result["total_steps"] > 0:
            execution_result["success_rate"] = execution_result["steps_executed"] / execution_result["total_steps"]
    ...
    async def run_full_test(self):
        ...
        for i, prompt_data in enumerate(test_prompts, 1):
            ...
            # Test execution simulation if plan was created successfully
            if result["success"]:
                execution_result = await self.test_execution_simulation(result["plan"])
                result["execution"] = execution_result
                
                print(f"🎯 Execution Simulation:")
                print(f"   Steps: {execution_result['steps_executed']}/{execution_result['total_steps']}")
                print(f"   Success Rate: {execution_result['success_rate']:.1%}")
                print(f"   Estimated Time: {execution_result['execution_time']:.1f}s")
