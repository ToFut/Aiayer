...
# Step 4: Create/Update Universal Intelligent Automation Handler
printf "\n${GREEN}🧠 Creating Universal Intelligent Automation Handler...${RESET}\n"

# Create universal_intelligent_automation_handler.py if it doesn't exist
cat > universal_intelligent_automation_handler.py << 'EOF'
#!/usr/bin/env python3
"""
...
chmod +x universal_intelligent_automation_handler.py
printf "   ${GREEN}✅ Universal Intelligent Automation Handler created${RESET}\n"
...
if [ "$BACKEND_OK" = true ]; then
    ...
    # Check if the file exists in sensors directory
    if [ -f "sensors/process_sensor.py" ]; then
        ...
        printf "   ${YELLOW}⚠️ Process sensor not found, skipping...${RESET}\n"
    fi
    ...
    if [ -f "sensors/total_screen_analyzer.py" ]; then
        ...
        printf "   ${YELLOW}⚠️ Total Screen Analyzer not found, skipping...${RESET}\n"
    fi
fi
