            ...
            try:
                ...
                if msg_type == 'agent_confirmation':
                    ...
                    if action == 'DO' or action == 'EXECUTE':
                        ...
                        # First check if session_id is actually a plan ID
                        plan_data = None
                        if session_id:
                            plan_data = await load_plan_by_id(session_id)
                        
                        # If no plan found with the session ID, try to find the newest plan
                        if not plan_data:
                            logger.warning(f"No plan found with ID: {session_id}, trying to find newest plan")
                        ...
                        if plan_data:
                            ...
                            # No plan found, send failure message
                ...
                # CRITICAL: Also support button_action message type as alternative format
                    ...
                    if action == 'EXECUTE_PLAN' or action == 'DO':
                        # Try to load the specified plan
                        plan_data = None
                        if plan_id:
                            plan_data = await load_plan_by_id(plan_id)
                        
                        # If no plan found with the given ID, try to find the newest plan
                        if not plan_data:
                            logger.warning(f"No plan found with ID: {plan_id}, trying to find newest plan")
                            newest_plan_id = await find_newest_plan()
...
if __name__ == "__main__":
    try:
        ...
        logger.info(f"Starting Real DO Button Executor (version 1.1)")
