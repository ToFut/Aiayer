#!/usr/bin/env python3
"""
Test Direct Chat Message to Overlay

This script sends a properly formatted chat message directly to the chat overlay,
using the expected format defined in EnterpriseChatWidget.svelte.
"""
...
async def send_direct_chat_message():
    """Send a direct chat message to the overlay"""
    try:
        ...
        async with websockets.connect(WS_URI, ping_interval=5, ping_timeout=20) as websocket:
            ...
            # Create direct chat message in the format expected by EnterpriseChatWidget
            # This format matches what the component expects in its handleBackendMessage function
            message = {
                "success": True,
            ...
            logger.info(f"Sent direct chat message to overlay")
...
async def main():
    """Main function"""
    logger.info("🚀 Sending direct chat message to overlay")
