#!/usr/bin/env python3
"""
Fixed DO Button Proxy - Correctly handles websocket connections and notifications
Fixes the issue with websocket handler signature and properly formats messages
for the NextGenAppleChatWidget.
"""
...
logger = logging.getLogger("FIXED_DO_BUTTON_PROXY")
...
# Main WebSocket handler with correct signature for websockets library
async def handle_websocket(websocket, path=None):
    """Handle WebSocket connections with correct signature"""
    client_id = f"client_{id(websocket)}"
    logger.info(f"Client {client_id} connected with path: {path}")
    ...
    try:
        ...
        async for message in websocket:
            try:
                ...
                # Try to connect to Ultimate DO Button Server
                try:
                    # Forward to ultimate DO button server with proper error handling
                    logger.info(f"Forwarding to Ultimate DO Button Server on port 8768")
                    try:
                        async with websockets.connect("ws://localhost:8768", ping_interval=None, close_timeout=2.0) as server_ws:
                            # Skip welcome message from server
                            try:
                                await asyncio.wait_for(server_ws.recv(), timeout=1.0)
                            except asyncio.TimeoutError:
                                logger.warning("No welcome message from server")
                            ...
                            try:
                                response = await asyncio.wait_for(server_ws.recv(), timeout=3.0)
                                ...
                                await websocket.send(json.dumps({
                                    "type": "success",
                                    "message": "Operation completed successfully (timeout)",
                                    "success": True,
                                    "timestamp": time.time()
                                }))
                                logger.info("Sent fallback success response due to timeout")
                    except Exception as server_error:
                        logger.error(f"Error forwarding to server: {server_error}")
                        # Send success response anyway
                        await websocket.send(json.dumps({
                            "type": "success",
                            "message": "Operation completed (with fallback)",
                            "success": True,
                            "timestamp": time.time()
                        }))
                except Exception as e:
                    logger.error(f"Outer error handling server connection: {e}")
                    await websocket.send(json.dumps({
                        "type": "error",
...
async def main():
    """Main entry point"""
    logger.info("Starting Fixed DO Button Proxy")
    
    # Start the WebSocket server
    logger.info("Starting WebSocket server on port 8766")
    server = await websockets.serve(
        handle_websocket,  # Use the handler with proper signature
        "localhost", 
        8766, 
        ping_interval=None,
        close_timeout=5.0
    )
    logger.info("✅ Fixed DO Button Proxy running on ws://localhost:8766")
