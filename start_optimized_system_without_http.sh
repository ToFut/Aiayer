#!/bin/bash
# start_optimized_system_without_http.sh
# A comprehensive script that applies fixes and starts the optimized system without HTTP server

set -e
echo "=== Starting Optimized Aiayer System with Smart Batching (WebSocket Only) ==="
...
# Step 6: Save PIDs for easier management
echo "Step 6: Saving process IDs for management..."
mkdir -p pids
echo "$PROCESS_SENSOR_PID" > "pids/process_sensor.pid"
echo "$SCREEN_SENSOR_PID" > "pids/screen_sensor.pid"
echo "$FILE_SENSOR_PID" > "pids/file_sensor.pid"
...
echo "=== Optimized System Started Successfully (WebSocket Only) ==="
...
echo ""
echo "IMPORTANT: The WebSocket server is now running on port 8767"
