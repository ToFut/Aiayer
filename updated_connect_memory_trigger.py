#!/usr/bin/env python3
"""
Updated Memory Trigger to Chat Overlay Connector

This script bridges the gap between the memory trigger service and the chat overlay
by listening for notification events and forwarding them to the correct WebSocket server.
Now connecting to port 8768 (fixed_bridge_server.py) instead of 8765.
"""
...
# WebSocket connection info - UPDATED PORT
WS_URI = "ws://localhost:8768"  # Changed from 8765 to 8768 to match EnterpriseChatWidget.svelte
...
class MemoryTriggerConnector:
    ...
    async def connect(self):
        ...
        try:
            ...
            # Connect with simpler parameters for better compatibility
    ...
    async def _notification_callback(self, notification):
        ...
        try:
            # Create direct chat message in the format expected by fixed_bridge_server.py
            direct_message = {
                "type": "do_button",
                "action": "display",
                "content": {
            ...
            # Send message to WebSocket - check if connection is active
            if self.websocket and not self.websocket.closed:
                ...
                if connected and self.websocket and not self.websocket.closed:
                    await self.websocket.send(json.dumps(direct_message))
...
async def main():
    ...
    logger.info("🚀 Starting memory trigger connector (UPDATED VERSION with port 8768)")
    ...
    # Verify WebSocket server is running on port 8768
    try:
        logger.info("Checking if WebSocket server is running on port 8768...")
        test_websocket = await websockets.connect("ws://localhost:8768")
        await test_websocket.close()
        logger.info("✅ WebSocket server is running on port 8768")
    except Exception as e:
        logger.error(f"❌ WebSocket server is not running on port 8768: {e}")
        logger.error("Please make sure overlay/fixed_bridge_server.py is running")
        logger.error("You can start it with: python overlay/fixed_bridge_server.py")
        
        # Create a simple guide for the user
        guide_message = """
TROUBLESHOOTING GUIDE:

1. Start WebSocket server on port 8768:
