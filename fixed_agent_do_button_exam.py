#!/usr/bin/env python3
"""
Fixed version of the advanced test script for the DO button examination system
This version addresses WebSocket compatibility issues and ensures proper execution
"""
import asyncio
import websockets
import json
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/exams/fixed_do_button_exam.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger('fixed_do_button_exam')
...
# FIXED WEBSOCKET HANDLER: updated parameter signature to match websockets lib expectations
async def websocket_handler(websocket, path=None):
    ...
    await websocket.send(json.dumps({
        "type": "welcome",
        "message": "Connected to Fixed DO Button Exam Server",
...
def save_test_results():
    ...
    try:
        if test_results:
            ...
            filename = f"logs/exams/results/fixed_exam_results_{timestamp}.json"
...
async def main():
    parser = argparse.ArgumentParser(description='Fixed DO Button Exam Server')
    ...
    logger.info(f"Starting Fixed WebSocket server on {args.host}:{args.port}")
    ...
    try:
        ...
        async with websockets.serve(
            ...
            if args.open_browser:
                ...
                html_path = os.path.join(current_dir, "fixed_agent_do_button_exam.html")
                
                # Check if the file exists, if not create a basic version
                if not os.path.exists(html_path):
                    # Copy the existing HTML file if it exists
                    original_html = os.path.join(current_dir, "advanced_agent_do_button_exam.html")
                    if os.path.exists(original_html):
                        import shutil
                        shutil.copy(original_html, html_path)
                        logger.info(f"Copied existing HTML interface to {html_path}")
                    else:
                        logger.info(f"Creating basic HTML interface at {html_path}")
                        with open(html_path, 'w') as f:
                            # Create a simplified version of the HTML file with the correct port
                            f.write(f"""<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Fixed Agent DO Button Exam</title>
    <style>
        ...
        button {{ background-color: #4caf50; color: white; border: none; padding: 10px 16px; cursor: pointer; border-radius: 4px; }}
...
<body>
    <div class="container">
        <h1>Fixed Agent DO Button Exam</h1>
        ...
        <div>
            <h2>Test Controls</h2>
            <div>
                <textarea id="messageInput" rows="4" style="width: 100%; margin-bottom: 10px;"></textarea>
                <button id="sendBtn">Send Message</button>
                <button id="doBtn" disabled>DO Button</button>
                <button id="dismissBtn" disabled>Dismiss</button>
            </div>
        </div>
        
        <h2>Server Response</h2>
        <div id="planDisplay" style="background-color: #f5f5f5; padding: 10px; border-radius: 4px; margin-bottom: 20px; font-family: monospace; white-space: pre-wrap;"></div>
    ...
    <script>
        ...
        function connect() {{
            ws = new WebSocket('ws://localhost:{args.port}');
            ...
            ws.onmessage = function(event) {{
                ...
                addToLog(`Received: ${{JSON.stringify(data, null, 2)}}`);
                
                // Handle plan_created message
                if (data.type === 'plan_created') {{
                    planDisplay.textContent = JSON.stringify(data.plan, null, 2);
        ...
        sendBtn.addEventListener('click', function() {{
            if (!ws || ws.readyState !== WebSocket.OPEN) {{
                addToLog('Not connected to WebSocket server');
                return;
            }}
            ...
            const data = {{
                ...
                exam_id: 'browser_search',  // Default to browser search exam
                timestamp: new Date().toISOString()
            }};
            
            // Send message
            ws.send(JSON.stringify(data));
            addToLog(`Sent: ${{JSON.stringify(data, null, 2)}}`);
        }});
        ...
        doBtn.addEventListener('click', function() {{
            ...
            // Send message
            ws.send(JSON.stringify(data));
            addToLog(`Sent DO: ${{JSON.stringify(data, null, 2)}}`);
        }});
        ...
        dismissBtn.addEventListener('click', function() {{
            ...
            // Send message
            ws.send(JSON.stringify(data));
            addToLog(`Sent DISMISS: ${{JSON.stringify(data, null, 2)}}`);
        ...
        window.addEventListener('load', connect);
            ...
            logger.info(f"Fixed DO Button Exam Server running at ws://{args.host}:{args.port}")
