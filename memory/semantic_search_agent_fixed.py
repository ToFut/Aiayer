                ...
                if count > 0:
                    ...
                    for row in cursor.fetchall():
                        try:
                            ...
                            logger.error(f"Error loading embedding for document {row[0]}: {e}")
                    
                    # Verify integrity
                    missing_embeddings = set(self.documents.keys()) - set(self.embeddings.keys())
                    if missing_embeddings:
                        logger.warning(f"Found {len(missing_embeddings)} documents without embeddings")
    ...
    def add_document(self, document: MemoryDocument, embedding: np.ndarray) -> bool:
        ...
        try:
            with self.index_lock:
                # Ensure embedding is not all zeros
                if np.all(embedding == 0):
                    logger.warning(f"Attempted to add document {document.id} with all-zero embedding")
                    return False
                ...
                if norm > 0:
                    ...
                    logger.warning(f"Document {document.id} has zero-norm embedding")
                    return False
...
class SemanticSearchAgent:
    ...
    def __init__(self, vector_store: Optional[EnterpriseVectorStore] = None):
        ...
        # Initialize word vectors for enhanced embedding
    ...
    def _initialize_word_vectors(self):
        """Initialize basic word vectors for enhanced embedding"""
        # Creating a simple word2vec-like dictionary for common words
        # In a production system, this would be loaded from a pre-trained model
        self.word_vectors = {}
        
        # Generate vectors for common words with some semantic meaning
        # Programming/development related terms
        for i, word in enumerate(['code', 'program', 'develop', 'software', 'debug', 
                                'python', 'java', 'javascript', 'function', 'class',
                                'api', 'server', 'client', 'database', 'algorithm']):
            v = np.zeros(1000)
            v[0:100] = np.random.normal(0.5, 0.1, 100)  # Programming dimension
            v[i*3:(i+1)*3] = np.random.normal(0.7, 0.1, 3)  # Word-specific dimension
            self.word_vectors[word] = v / np.linalg.norm(v)  # Normalize
        
        # Research/documentation related terms
        for i, word in enumerate(['research', 'paper', 'document', 'study', 'analyze',
                                'article', 'report', 'data', 'experiment', 'science']):
            v = np.zeros(1000)
            v[100:200] = np.random.normal(0.5, 0.1, 100)  # Research dimension
            v[300+i*3:300+(i+1)*3] = np.random.normal(0.7, 0.1, 3)  # Word-specific dimension
            self.word_vectors[word] = v / np.linalg.norm(v)  # Normalize
            
        # Professional context related terms
        for i, word in enumerate(['work', 'business', 'professional', 'project', 'task',
                                'meeting', 'presentation', 'manager', 'team', 'productivity']):
            v = np.zeros(1000)
            v[200:300] = np.random.normal(0.5, 0.1, 100)  # Professional dimension
            v[600+i*3:600+(i+1)*3] = np.random.normal(0.7, 0.1, 3)  # Word-specific dimension
            self.word_vectors[word] = v / np.linalg.norm(v)  # Normalize
    ...
    def _compute_tfidf_embedding(self, text: str) -> np.ndarray:
        ...
        # Add TF-IDF components for all tokens (statistical dimension)
        for token, count in token_counts.items():
            # Calculate TF (term frequency)
            tf = count / len(tokens)
            
            # Calculate IDF (inverse document frequency)
            denominator = self.document_frequencies[token] + 1  # Smoothing
            if self.total_documents == 0:
                idf = 1.0  # Default for empty index
            else:
                idf = np.log((self.total_documents + 1) / denominator)  # Smoothed IDF
            
            # Create positional encoding to differentiate tokens
            position = hash(token) % 700  # Positional encoding based on token hash
            if position < embedding.shape[0]:
                embedding[position] += tf * idf
    ...
    async def add_memory(self, content: str, source: str = "user", 
        ...
        try:
            ...
            # Compute embedding with improved method
            embedding = self._compute_tfidf_embedding(content)
            
            # Verify embedding quality
            if np.all(embedding == 0):
                logger.warning(f"Generated zero embedding for content: {content[:50]}...")
                # Fall back to a basic embedding if needed
                embedding = np.random.normal(0, 0.1, 1000)
    ...
    async def search_memories(self, query: str, 
        ...
        try:
            ...
            # Enhanced query with application context if provided
            ...
            cache_key = self._get_cache_key(enhanced_query, filters)
            ...
            query_embedding = self._compute_tfidf_embedding(enhanced_query)
            
            # Ensure query embedding is valid
            if np.all(query_embedding == 0):
                logger.warning(f"Zero query embedding for: '{enhanced_query}'")
                # Create a basic query vector
                query_embedding = np.random.normal(0, 0.1, 1000)
            ...
            for doc_id, similarity in similar_docs:
                ...
                if document:
                    ...
                    relevance_factors = self._get_relevance_factors(enhanced_query, document, similarity)
                    
                    # Apply application context boost if provided
                    final_similarity = similarity
                    if application_context and application_context.lower() in document.content.lower():
                        final_similarity *= 1.2  # Boost score by 20% for context match
                        relevance_factors.append("application_context_match")
    ...
    async def get_context_for_query(self, query: str, 
                                  ...
                                  application_context: Optional[str] = None) -> Dict[str, Any]:
