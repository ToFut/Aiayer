#!/usr/bin/env python3
"""
Advanced Bridge Server with Agent Automation
Connects the frontend overlay with AI backend, memory systems, and agent automation.
"""
import asyncio
import json
import logging
...
# Import agent automation components
sys.path.append(os.path.dirname(os.path.abspath(__file__)))

# Global flag for automation availability
...
try:
    ...
    print(f"Warning: Agent automation modules not available: {e}")
    AUTOMATION_AVAILABLE = False
...
class AdvancedBridge:
    ...
    def __init__(self, frontend_port=8765, backend_port=8766):
        ...
        self.message_handlers = {
            ...
            'action_confirmation': self.handle_action_confirmation,
            'emergency_stop': self.handle_emergency_stop
        }
        
        # Initialize agent automation if available
        self.agent = None
        ...
        if self.automation_available:
            try:
                ...
                logger.info("🤖 Agent automation initialized")
                logger.info("🚨 Emergency shutdown: Press Ctrl+1 to stop agent immediately")
            except Exception as e:
                logger.error(f"Failed to initialize agent automation: {e}")
    ...
    async def handle_backend_client(self, websocket):
    ...
    async def handle_action_confirmation(self, websocket: WebSocketServerProtocol, payload: Dict[str, Any], source: str):
        """Handle automation action confirmation."""
        if not self.automation_available or not self.input_controller:
            await self.send_to_client(websocket, 'automation_failed', {
        ...
        if confirmed:
            ...
            await self.execute_automation_action(websocket, action_id)
        else:
            # User declined
            await self.send_to_client(websocket, 'automation_cancelled', {
                'message': f'Action declined by user',
                'action_id': action_id
            })
    
    async def handle_emergency_stop(self, websocket: WebSocketServerProtocol, payload: Dict[str, Any], source: str):
        ...
        await self.broadcast_to_frontends(emergency_message)
        
        logger.critical("🛑 All automation stopped")
    ...
    async def execute_automation_action(self, websocket: WebSocketServerProtocol, action_id: str):
        """Execute an approved automation action."""
        try:
            # Send execution start notification
            await self.send_to_client(websocket, 'automation_started', {
                'message': f'🤖 Executing automation...',
                'action_id': action_id
            })
            
            # For now, simulate some basic actions
            # In a real implementation, you'd store the action details and execute them
            ...
            # Send result
            if success:
                await self.send_to_client(websocket, 'automation_completed', {
                ...
                await self.send_to_client(websocket, 'automation_failed', {
                    'message': f'❌ Failed to execute automation',
                    'action_id': action_id,
                    'success': False
                })
